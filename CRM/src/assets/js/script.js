...
$(function () {
    ...
    /* If has className sub-menu-opened */
    ...
    jQuery('img.svg').each(function () {
        ...
        var imgClass = $img.attr('className');
        ...
        jQuery.get(imgURL, function (data) {
            ...
            if (typeof imgClass !== 'undefined') {
                $svg = $svg.attr('className', imgClass + ' replaced-svg');
            }
    ...
    if(customCheckboxes.length) {
        ...
        $('.project-box, .file').find('.dropdown-button .delete').on('click', function(e) {
            e.preventDefault();
            $(this).parents().filter( '[className*="col-"]' ).remove();
        });
    }
    ...
    if($scrollBar.length) {
        $scrollBar.each(function () {
            ...
            $pos = localStorage.getItem( 'ps.' + this.classNameList[0] );
        ...
        $scrollBar.on('ps-scroll-y', function () {
            localStorage.setItem( 'ps.' + this.classNameList[0], this.scrollTop );
        });
    }
    ...
    $('.file').find('.dropdown-button .detete').on('click', function() {
        $(this).parents('.file').parent('[className^="col-"]').remove();
    });
        ...
        window.onload = function() {
            setTimeout(function() {
                ...
                <div className="base-timer">
                    ...
                    <span className="base-timer__text">Seconds</span>
                </div>
                `;
                ...
                function setRemainingPathColor(timeLeft) {
                    ...
                    if (timeLeft <= alert.threshold) {
                        document.getElementById("base-timer-path-remaining").classNameList.remove(warning.color);
                        document.getElementById("base-timer-path-remaining").classNameList.add(alert.color);
                    } else if (timeLeft <= warning.threshold) {
                        document.getElementById("base-timer-path-remaining").classNameList.remove(info.color);
                        document.getElementById("base-timer-path-remaining").classNameList.add(warning.color);
                    }
                }
